...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, poses_name=None):
    if poses_name == None:
        poses_arr = np.load(os.path.join(basedir, 'poses_bounds.npy'))
    else:
        poses_arr = np.load(os.path.join(basedir, poses_name))
...
def load_subframe_poses(args, basedir, bd_factor=.75, recenter=True):
    poses_arr = np.load(os.path.join(basedir, 'poses_bounds_subframes.npy'))
    
    n_imgs, n_subframes, _ = poses_arr.shape
    poses_sub = poses_arr[:, :, :-2].reshape([n_imgs, n_subframes, 3, 5])[:, :, :, :4]
    poses_sub = np.concatenate([poses_sub[:, :, :, 1:2], -poses_sub[:, :, :, 0:1], poses_sub[:, :, :, 2:]], -1)

    poses_arr = np.load(os.path.join(basedir, 'poses_bounds.npy'))
    poses = poses_arr[:, :-2].reshape([-1, 3, 5])[:, :, :4]
    ...
    if recenter:
        ...
        poses_sub_ = poses_sub + 0
        bottom = np.tile(bottom[:, None], [1, n_subframes, 1, 1])
        poses_sub = np.concatenate([poses_sub, bottom], -2)
        poses_sub = np.linalg.inv(c2w) @ poses_sub.reshape(-1, 4, 4)
        poses_sub = poses_sub.reshape(n_imgs, n_subframes, 4, 4)

        poses_sub_[:, :, :3, :4] = poses_sub[:, :, :3, :4]
        poses_sub = poses_sub_
