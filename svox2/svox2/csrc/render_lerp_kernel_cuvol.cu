...
__device__ __inline__ void trace_ray_cuvol(
    ...
    const uint32_t lane_colorgrp_id = lane_id % grid.basis_dim; // idx of sh
    const uint32_t lane_colorgrp = lane_id / grid.basis_dim;    // r g or b

    const float background_brightness = lane_colorgrp == 0 ? opt.background_r : (lane_colorgrp == 1 ? opt.background_g : opt.background_b);
    if (ray.tmin > ray.tmax) {
        out[lane_colorgrp] = (grid.background_nlayers == 0) ? background_brightness : 0.f;
...
#pragma unroll 3
    ...
    if (grid.background_nlayers == 0) {
        outv += _EXP(log_transmit) * background_brightness;
    }
...
__device__ __inline__ void trace_ray_cuvol_backward(
        ...
        float* __restrict__ dhat_d,
        float* __restrict__ grad_rays_o,
        float* __restrict__ grad_rays_d,
        WarpReducef::TempStorage& __restrict__ temp_storage,
        WarpReducef::TempStorage* __restrict__ temp_storage_rays_o,
        WarpReducef::TempStorage* __restrict__ temp_storage_rays_d,
        float log_transmit_in,
        float beta_loss,
        float sparsity_loss,
        PackedGridOutputGrads& __restrict__ grads,
        float* __restrict__ grad_sph_over_dir,
    ...
    float log_transmit = 0.f;
    float weight_side_cumul_grad[3] = {0.f,} , weight_side_cumul_dgrad[3] = {0.f,};
    float tlerp_grad[3], tlerp_dgrad[3];
...
#pragma unroll 3
        ...
        if (sigma > opt.sigma_thresh) {
            ...
            // [Taekkii] Color-side graident over rays
            trilerp_backward_cuvol_one_point_side(
                    grid.links,
                    grid.sh_data,
                    grid.stride_x,
                    grid.size[2],
            ...
            float c_side_grad_o[3], c_side_grad_d[3];
#pragma unroll 3
            for(int j = 0; j < 3; j++) {
                const float grad_o = sphfunc_val[lane_colorgrp_id]*tlerp_grad[j];
                const float grad_d = sphfunc_val[lane_colorgrp_id]*tlerp_dgrad[j] + 
                                     lane_color*grad_sph_over_dir[9*j + lane_colorgrp_id];
                                       
                c_side_grad_o[j] = WarpReducef(temp_storage_rays_o[j]).HeadSegmentedSum( 
                                    grad_o,
                                    lane_colorgrp_id == 0);

                c_side_grad_d[j] = WarpReducef(temp_storage_rays_d[j]).HeadSegmentedSum(
                                   grad_d, 
                                   lane_colorgrp_id == 0);
            }
...
#pragma unroll 3
                for(int j = 0; j < 3 ; j++){
                    atomicAdd(&grad_rays_o[j],gout * weight * color_in_01 * c_side_grad_o[j] * grid._scaling[j]);
                    atomicAdd(&grad_rays_d[j],gout * weight * color_in_01 * c_side_grad_d[j]);
                }
            }
            ...
            if(lane_colorgrp_id == 0){
                // [Taekkii] Weight-side graident over rays
                trilerp_backward_cuvol_one_point_side(
                        grid.links,
                        grid.density_data,
                        grid.stride_x,
                        ...
                        0,
                        tlerp_grad);
                get_tlerp_dgrad(dhat_d,
                                tlerp_grad,
                                t,
                                //output
                                tlerp_dgrad);
                const float ci = lane_color_total * color_in_01;
#pragma unroll 3
                for(int j = 0; j < 3; j++) {
                    atomicAdd(&grad_rays_o[j],(-weight_side_cumul_grad[j] * weight +
                                       _EXP(log_transmit + pcnt) * _EXP(-pcnt) * ray.world_step * tlerp_grad[j])
                                      * gout * ci * grid._scaling[j]);
                    atomicAdd(&grad_rays_d[j],(-weight_side_cumul_dgrad[j] * weight +
                                       _EXP(log_transmit + pcnt) * _EXP(-pcnt) * ray.world_step * tlerp_dgrad[j])
                                      * gout * ci);
                    weight_side_cumul_grad[j] += ray.world_step * tlerp_grad[j];
                    weight_side_cumul_dgrad[j] += ray.world_step * tlerp_dgrad[j];
                }
                // [Taekkii] Weight-side graident over rays ends
            }
            if (_EXP(log_transmit) < opt.stop_thresh) {
                break;
            }
...
__global__ void render_ray_backward_kernel(
    ...
    __shared__ float grad_sph_over_dir[TRACE_RAY_BKWD_CUDA_RAYS_PER_BLOCK][3*9];
    __shared__ float dhat_d[TRACE_RAY_BKWD_CUDA_RAYS_PER_BLOCK][3*3];
    ...
    // [Taekki adds below on 12/26] more tempstorage for ray grads
    __shared__ typename WarpReducef::TempStorage temp_storage_rays_o[TRACE_RAY_BKWD_CUDA_RAYS_PER_BLOCK][3];
    __shared__ typename WarpReducef::TempStorage temp_storage_rays_d[TRACE_RAY_BKWD_CUDA_RAYS_PER_BLOCK][3];
    ...
    if (lane_id == 0) { 
        get_dhat_d(ray_spec[ray_blk_id], grid, opt, dhat_d[ray_blk_id]);
        ray_find_bounds(ray_spec[ray_blk_id], grid, opt, ray_id);
        calc_sh_backward_dx(vdir, &grad_sph_over_dir[ray_blk_id][0]);
        calc_sh_backward_dy(vdir, &grad_sph_over_dir[ray_blk_id][9]);
        calc_sh_backward_dz(vdir, &grad_sph_over_dir[ray_blk_id][18]);
    }
...
#pragma unroll 3
    ...
    trace_ray_cuvol_backward(
        ...
        dhat_d[ray_blk_id],
        &grads.grad_rays_o_out[ray_id*3],
        &grads.grad_rays_d_out[ray_id*3],
        temp_storage[ray_blk_id],
        temp_storage_rays_o[ray_blk_id],
        temp_storage_rays_d[ray_blk_id],
        log_transmit_in == nullptr ? 0.f : log_transmit_in[ray_id],
        beta_loss,
        sparsity_loss,
        grads,
        grad_sph_over_dir[ray_blk_id],
...
void volume_render_cuvol_fused_backward(
        SparseGridSpec& grid,
        RaysSpec& rays,
        RenderOptions& opt,
        torch::Tensor rgb_gt,
