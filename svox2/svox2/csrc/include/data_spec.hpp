...
struct GridOutputGrads {
  ...
  torch::Tensor grad_rays_o_out;
  torch::Tensor grad_rays_d_out;
  ...
  inline void check() {
    ...
    if (grad_rays_o_out.defined()){
      CHECK_INPUT(grad_rays_o_out)
    }
    if (grad_rays_d_out.defined()){
      CHECK_INPUT(grad_rays_d_out)
    }
