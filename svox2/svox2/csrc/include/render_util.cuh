...
// trilerp for point_gradient side
// returns derivative del_sh-coefficient/del_[] where [] is one of {px, py, pz} 
template<class data_type_t, class voxel_index_t>
__device__ __inline__ void trilerp_backward_cuvol_one_point_side(
        const int32_t* __restrict__ links,
        const data_type_t* __restrict__ data,
        int offx, int offy, size_t stride,
        const voxel_index_t* __restrict__ l,
    ...
    // result must be an array of size 3 [dq/dx, dq/dy, dq/dz]
    const int32_t* __restrict__ link_ptr = links + (offx * l[0] + offy * l[1] + l[2]);
...
#define MAYBE_READ_LINK(u) ((link_ptr[u] >= 0) ? data[link_ptr[u] * stride + idx] : 0.f)
    ...
    const float iy0 = lerp(ix0y0, ix1y0, pos[0]);
    const float iy1 = lerp(ix0y1, ix1y1, pos[0]);
    result[1] = iy1 - iy0;

    const float iy0z0 = lerp(MAYBE_READ_LINK(0), MAYBE_READ_LINK(offx), pos[0]);
    const float iy0z1 = lerp(MAYBE_READ_LINK(1), MAYBE_READ_LINK(offx + 1), pos[0]);
    const float iy1z0 = lerp(MAYBE_READ_LINK(offy), MAYBE_READ_LINK(offy + offx), pos[0]);
    const float iy1z1 = lerp(MAYBE_READ_LINK(offy + 1), MAYBE_READ_LINK(offy + offx + 1), pos[0]);
    
    const float iz0 = lerp(iy0z0, iy1z0, pos[1]);
    const float iz1 = lerp(iy0z1, iy1z1, pos[1]);
    result[2] = iz1 - iz0;
...
// [Taekkii] sh coefficients have gradients over direction vector of a ray, so we need below 3 functions
__device__ __inline__ void calc_sh_backward_dx(
    const float* __restrict__ dir,
    float* __restrict__ out) {
    const float x = dir[0], y = dir[1], z = dir[2];

    out[0] = out[1] = out[2] = out[5] = 0.;
    out[3] = -C1;
    out[4] = C2[0] * y;
    out[6] = -2.0 * C2[2] * x;
    out[7] = C2[3] * z;
...
__device__ __inline__ void calc_sh_backward_dy(
    const float* __restrict__ dir,
    float* __restrict__ out) {
    const float x = dir[0], y = dir[1], z = dir[2];

    out[0] = out[2] = out[3] = out[7] = 0.;
    out[1] = -C1;
    out[4] = C2[0] * x;
    out[5] = C2[1] * z;
    out[6] = -2.0 * C2[2] * y;
...
__device__ __inline__ void calc_sh_backward_dz(
    const float* __restrict__ dir,
    float* __restrict__ out) {
    const float x = dir[0], y = dir[1], z = dir[2];

    out[0] = out[1] = out[3] = out[4] = out[8] = 0.;
    out[2] = C1;
    out[5] = C2[1] * y;
    out[6] = 4.0 * C2[2] * z;
    out[7] = C2[3] * x;
...
// [Taekkii] Need this to take account for direction vector transformation
__device__ __inline__ void get_dhat_d(
    SingleRaySpec& __restrict__ ray,
    const PackedSparseGridSpec& __restrict__ grid,
    const RenderOptions& __restrict__ opt,
    // output
    float* __restrict__ dhat_d) {
    // dhat_d: 9 dim array, saves at: [dhat * 3 + d]
    // e.g. dhat_d[1*3+2] : dhat_y / dz
    //      dhat_d[2*3_0] : dhat_z / dx

    const float* k = grid._scaling;
    const float delta_scale = 1.0f / sqrtf(k[0]*k[0]*ray.dir[0]*ray.dir[0] + k[1]*k[1]*ray.dir[1]*ray.dir[1] + k[2]*k[2]*ray.dir[2]*ray.dir[2]);
    const float delta_s3 = delta_scale * delta_scale * delta_scale;

    for(int i = 0; i < 9; ++i) {
        const int dhat_i = i/3;
        const int d_i = i%3;
        dhat_d[i] = -k[dhat_i] * ray.dir[dhat_i] * (k[d_i]*k[d_i]) * ray.dir[d_i] * delta_s3;
        if(dhat_i == d_i) {
            dhat_d[i] += k[d_i] * delta_scale;
        }
    }
}
// calc dq/dx with dq/d(x_hat) and d(x_hat)/dx 
// (similar: y, z) 
__device__ __inline__ void get_tlerp_dgrad(
    const float* __restrict__ dhat_d,
    const float* __restrict__ tlerp_grad,
    const float t,
    // output
    float* tlerp_dgrad) {
#pragma unroll 3   
    ...
    for(int i = 0; i < 9; i++) {
        tlerp_dgrad[i%3] += t * tlerp_grad[i/3] * dhat_d[i];
    }
}
