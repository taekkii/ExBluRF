...
class RenderOptions:
    ...
    def _to_cpp(self, randomize: bool = False):
        ...
        opt.background_r = self.background_r;
        opt.background_g = self.background_g;
        opt.background_b = self.background_b;
...
class _VolumeRenderFunction(autograd.Function):
    ...
    def forward(
        ...
        data_rays_o: torch.Tensor,
        data_rays_d: torch.Tensor,
        beta_loss:float,
        sparsity_loss:float,
        grid,
        rays,
        ...
        """
        params:
            data_rays_o: added by taekkii in order to let gradient flow to ray-side
            data_rays_d: added by taekkii in order to let gradient flow to ray-side
        """
        ...
        ctx.beta_loss = beta_loss
        ctx.sparsity_loss = sparsity_loss
        return color
    ...
    def backward(ctx, grad_out):
        ...
        grad_density_grid = torch.zeros_like(ctx.grid.density_data.data)
        grad_sh_grid = torch.zeros_like(ctx.grid.sh_data.data)
        grad_rays_o = torch.zeros_like(ctx.rays.origins.data)
        grad_rays_d = torch.zeros_like(ctx.rays.dirs.data)
        ...
        grad_holder.grad_rays_o_out = grad_rays_o
        grad_holder.grad_rays_d_out = grad_rays_d
        ...
        return grad_density_grid, grad_sh_grid, grad_basis, grad_background, grad_rays_o, grad_rays_d,\
...
class SparseGrid(nn.Module):
    ...
    def volume_render(
        ...
        return_raylen: bool=False, beta_loss: float = 0.0, sparsity_loss: float = 0.0
    ):
        """
